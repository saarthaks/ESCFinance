...
import {SmallRules} from './PGSmallRules.js';
...
Template.PGSmallLayout.onCreated( function() {
    ...
    Session.set('pgsmallform', initForm());
    console.log(Session.get('pgsmallform'));
});
...
var saveFormSession = function(templateName) {
    switch (templateName) {
        case "PGSmallRules":
            const acceptTermsVar = $("[name='acceptTerms']").is(':checked');
            console.log('Saving status ' + acceptTermsVar.toString())
            var formFields = Session.get('pgsmallform');
            var oldStat = !!formFields['acceptTerms']
            console.log('Previous status: ' + oldStat.toString());
            formFields['acceptTerms'] = acceptTermsVar;
            Session.set('pgsmallform', formFields);
            formFields = Session.get('pgsmallform');
            console.log('New Status: ' + formFields['acceptTerms'].toString());
            break;
        case "PGContactForm":
            var names = [];
            var unis = [];
...
Template.PGSmallLayout.events({
    ...
    'submit form': function(e, template) {
        ...
        e.preventDefault();
        //testEmail();
        // var data = $("#pg-ss-form").searlizeObject();
        // submitForm(template);
        // console.log(data);
        submitPGForm();
